...
class Unet3_local(general_model):
    def _buildnet(self):
        self.varlist = None
        ...
        with tf.variable_scope("Unet", reuse=False, initializer=base_init, regularizer=reg_init):
            if self.FLAGS.unet_levels == 3:
                pred = unet3_l3(self.FLAGS.nfilters, x, y, deconv=False, is_training=self.FLAGS.train, is_pad=self.FLAGS.is_pad, varlist=self.varlist)
            elif self.FLAGS.unet_levels == 2:
                pred = unet3_l2(self.FLAGS.nfilters, x, y, deconv=False, is_training=self.FLAGS.train, is_pad=self.FLAGS.is_pad, varlist=self.varlist)
            elif self.FLAGS.unet_levels == 1:
                pred = unet3_l1(self.FLAGS.nfilters, x, y, deconv=False, is_training=self.FLAGS.train, is_pad=self.FLAGS.is_pad, varlist=self.varlist)
            elif self.FLAGS.unet_levels == 0:
                pred = simple_conv(self.FLAGS.nfilters, x, y, deconv=False, is_training=self.FLAGS.train, is_pad=self.FLAGS.is_pad, varlist=self.varlist)
            else:
        ...
        # self.loss = getSSIM(y, self.pred, single_layer=True) + reg
