...
def GRIBplslice(files, opathpre="plslice", types='t', level=850):
    #types = ['t', 'q', 'w', 'd', 'u', 'v', 'z']
    heights = [level]
    print(files)
    GRIBprocess(files, opathpre=opathpre, type_L=types, height_L=heights,
            type2index={types : 0}, level2index={level: 0})
...
if __name__ == "__main__":
    ...
    if slice_level == 500:
        types = 'z'
    elif slice_level == 850:
        types = 't'
    else:
        assert False, "The slice level has to be either 500 or 850"
    file_comment = "emos" + str(slice_level) + "slice48"
    files = getfilelist("plfl")
    PATHI = ensemble_GRIB_path
    PATHO = npy_output_path
    PATHTF = tfrecord_output_path
    XONLY_FLAG = False
    GRIBplslice(files, types=types, level=slice_level)
